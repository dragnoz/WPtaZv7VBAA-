scoreboard players set @s defence 0
scoreboard players set @s skill 0
scoreboard players set @s stamina 0
scoreboard players set @s focus 0

scoreboard players set @s combatstamina 0
scoreboard players set @s combatskill 0
scoreboard players set @s combatdefence 0
scoreboard players set @s combatfocus 0
say hello again


scoreboard players set @s  title 0
#runs logic factory
scoreboard players set @s  logic 0
#controls match length
scoreboard players set @s  round 0
#runs teleport factory
scoreboard players set @s  teleport 0
#used when player takes damage
scoreboard players set @s  points 0

scoreboard players set @s  combatvalues 0 

#used for combat controller
scoreboard players set @s  combat 0


titleraw @s[m=2] title {"rawtext":[{"text":"All control scoreboards reset"}]}
